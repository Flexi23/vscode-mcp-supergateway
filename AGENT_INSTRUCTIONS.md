...
*   **SiYuan Note:** `docker/gateway.config.json` configures the `siyuan-note` upstream, talking over REST to the `siyuan` Docker Compose service (image `b3log/siyuan`, port `6806`). Requires `SIYUAN_TOKEN` and `SIYUAN_ACCESS_AUTH_CODE` in `.env`, and `SIYUAN_WORKSPACE_DIR` (a host path bind-mounted as the SiYuan workspace) — see `.env.example`.
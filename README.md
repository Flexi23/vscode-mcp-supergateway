...
|---|---|---|---|
| `codebase-memory` | `codebase-memory-mcp` | Node | Auto-indexes the container-side `CBM_AUTO_INDEX_PATH` on startup (typically `/workspace`), uses `CBM_DEFAULT_PATH` for the routed workspace view, and exposes its graph through the same dashboard port `3100` while sharing `CBM_CACHE_DIR` with the UI process (`CBM_UI_PORT`/`CBM_UI_BACKEND_PORT` are reserved for a future standalone UI but unused today). |
